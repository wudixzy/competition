...
| `BI100_ATTN_COREX_FUSED_PREFILL` | `0` (service-rejected) | boolean | Experimental hash-pinned M1-47 split4 prefill path for the causal decoder, single-sequence, FP16 TP4 rank-local `4/1/256`, block-16 shape. It passed the isolated core gate but failed the fixed production cold-TTFT gate and must remain disabled. | M1-47 |
