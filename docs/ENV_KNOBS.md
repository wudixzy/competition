...
| `BI100_GDN_COREX_PACKED_DECODE` | `0` | boolean | Enables the experimental single-token TP4-local packed q/k/v, beta/decay, recurrent update, and output kernel for the exact `(1,4,8,128)` GDN shape; unsupported inputs use the existing path. | E-GDN-14 |
