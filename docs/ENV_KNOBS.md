...
| `BI100_CPU_KV_TRANSFER_LAYOUT` | `paged` | `paged`, `block_major` | Selects the private M1-46 CPU-tier data plane. `block_major` requires `BI100_CPU_KV_OFFLOAD=1`, the fixed Qwen3.6 TP4 rank-local shape, and the prebuilt CoreX pack/scatter extension. It remains forbidden in submission YAML until transfer, model, capacity, and 881-request gates qualify. | M1-46 |
