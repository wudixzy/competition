...
| `BI100_GDN_RESTORE_MODE` | `direct` | `direct`, `aligned` | `direct` restores the longest exact stable state. `aligned` admits only boundaries divisible by `max_num_batched_tokens`; it is a fallback experiment, not the submission default. | M1-31 |
