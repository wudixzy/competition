...
OUTPUT_DIR=${1:?usage: build_corex_fused_paged_prefill_batched_split_pv.sh OUTPUT_DIR}
COREX_ROOT=${COREX_ROOT:-/usr/local/corex-3.2.3}
TORCH_ROOT=${TORCH_ROOT:-${COREX_ROOT}/lib64/python3/dist-packages/torch}
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT=${OUTPUT_DIR}/corex_fused_paged_prefill_batched_split_pv.so
...
test -s "${OUTPUT}"
printf '[ok] CoreX batched split-PV fused prefill extension %s\n' "${OUTPUT}"
