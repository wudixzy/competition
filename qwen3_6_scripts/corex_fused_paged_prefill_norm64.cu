...
constexpr int kNormalizeThreads = 64;
constexpr int kNormalizeLogicalLanes = 256;
...
__global__ void normalize_split_scores_kernel(
  ...
  __shared__ float reduction[kNormalizeLogicalLanes];
  ...
  for (int split = 0; split < active_splits; ++split) {
    ...
    for (int logical_lane = threadIdx.x;
         logical_lane < kNormalizeLogicalLanes;
         logical_lane += blockDim.x) {
      float local_max = -std::numeric_limits<float>::infinity();
      for (int column = logical_lane; column < kTileTokens;
           column += kNormalizeLogicalLanes) {
        local_max = fmaxf(local_max, row_scores[column]);
      }
      reduction[logical_lane] = local_max;
    }
    __syncthreads();
    for (int stride = kNormalizeLogicalLanes / 2;
         stride > 0; stride /= 2) {
      for (int logical_lane = threadIdx.x;
           logical_lane < stride;
           logical_lane += blockDim.x) {
    ...
    for (int logical_lane = threadIdx.x;
      ...
      reduction[logical_lane] = local_sum;
    }
    __syncthreads();
    for (int stride = kNormalizeLogicalLanes / 2;
         stride > 0; stride /= 2) {
      for (int logical_lane = threadIdx.x;
           logical_lane < stride;
           logical_lane += blockDim.x) {
        reduction[logical_lane] = __fadd_rn(
            reduction[logical_lane], reduction[logical_lane + stride]);
      }
      __syncthreads();
    }
...
std::vector<torch::Tensor> fused_paged_prefill_forward(
  ...
  const auto run_group = [&](int group_start, int group_tokens,
    ...
    normalize_split_scores_kernel<<<rows, kNormalizeThreads, 0, stream>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def("forward", &fused_paged_prefill_forward,
             "Fixed-shape FP32 paged-prefill with one-warp normalization");
}
