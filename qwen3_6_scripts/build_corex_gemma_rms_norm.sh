...
"${COREX_ROOT}/bin/clang++" \
    ...
    -DTORCH_EXTENSION_NAME=corex_gemma_rms_norm \
    ...
    "${SCRIPT_DIR}/corex_gemma_rms_norm.cu" \
    -L"${TORCH_ROOT}/lib" \
    -L"${COREX_ROOT}/lib64" \
...
test -s "${OUTPUT}"
printf '[ok] CoreX Gemma RMSNorm extension %s\n' "${OUTPUT}"
