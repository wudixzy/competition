...
#else
    prepared[index] = __half2float(query[source]) * scale;
#endif
  }
}
...
__global__ void gather_kv_group_kernel(
    ...
    const int* block_table, KeyTile* key_tiles, float* value_tiles,
...
#if BI100_HALF_INPUT_QK
    __half key_value = __float2half(0.0f);
#else
...
cublasStatus_t qk_batched(
    cublasHandle_t handle, const KeyTile* key_tile, const KeyTile* query,
    float* scores, int query_len, float scale) {
#if BI100_HALF_INPUT_QK
  ...
  return cublasGemmStridedBatchedEx(
      ...
      &alpha, key_tile, CUDA_R_16F, kHeadDim, 0,
      query, CUDA_R_16F, kHeadDim,
      static_cast<long long>(query_len) * kHeadDim,
      &beta, scores, CUDA_R_32F, kTileTokens,
...
std::vector<torch::Tensor> fused_paged_prefill_forward(
...
#if BI100_HALF_INPUT_QK
  auto prepared_query = torch::empty(
      {kNumQueryHeads, query_len, kHeadDim}, query.options());
  auto key_tiles = torch::empty(
      {kSplitCount, kTileTokens, kHeadDim}, query.options());
#else
  auto prepared_query = torch::empty(
  ...
  prepare_query_kernel<<<launch_blocks(output_elements), kThreads, 0, stream>>>(
      reinterpret_cast<const __half*>(query.data_ptr<at::Half>()),
#if BI100_HALF_INPUT_QK
      reinterpret_cast<__half*>(prepared_query.data_ptr<at::Half>()),
#else
      prepared_query.data_ptr<float>(),
#endif
      query_len,
  ...
  const auto run_group = [&](int group_start, int group_tokens,
    ...
    for (int split = 0; split < active_splits; ++split) {
      check_cublas(qk_batched(
          handle,
#if BI100_HALF_INPUT_QK
          reinterpret_cast<const __half*>(key_tiles.data_ptr<at::Half>())
              + split * key_split_stride,
          reinterpret_cast<const __half*>(prepared_query.data_ptr<at::Half>()),
#else
          key_tiles.data_ptr<float>() + split * key_split_stride,
          prepared_query.data_ptr<float>(),
#endif
          scores.data_ptr<float>() + split * score_split_stride,
          query_len, static_cast<float>(scale_arg)),
          "split4 paged prefill QK");
    }
