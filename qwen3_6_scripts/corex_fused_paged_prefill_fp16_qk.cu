...
__global__ void pack_query_kernel(const __half* query, __half* packed,
                                  int query_len) {
...
__global__ void gather_kv_group_kernel(
    ...
    const int* block_table, __half* key_tiles, float* value_tiles,
...
cublasStatus_t qk_batched(
    cublasHandle_t handle, const __half* key_tile, const __half* query,
  ...
  return cublasGemmStridedBatchedEx(
      ...
      &alpha,
      key_tile, CUDA_R_16F, kHeadDim, 0,
...
std::vector<torch::Tensor> fused_paged_prefill_forward(
  ...
  auto half_options = query.options().dtype(torch::kFloat16);
  ...
  pack_query_kernel<<<launch_blocks(output_elements), kThreads, 0, stream>>>(
      reinterpret_cast<const __half*>(query.data_ptr<at::Half>()),
      reinterpret_cast<__half*>(packed_query.data_ptr<at::Half>()),
      query_len);
  ...
  const auto run_group = [&](int group_start, int group_tokens,
    ...
    for (int split = 0; split < active_splits; ++split) {
      check_cublas(qk_batched(
          ...
          reinterpret_cast<const __half*>(packed_query.data_ptr<at::Half>()),
          scores.data_ptr<float>() + split * score_split_stride,
          query_len, static_cast<float>(scale_arg)),
          "split4 FP16-input paged prefill QK");
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def("forward", &fused_paged_prefill_forward,
             "Fixed-shape FP16-QK/FP32-accumulate paged-prefill pipeline");
}
