...
build_stage "building fused CoreX GDN beta and decay extension"
bash ./build_corex_gdn_beta_decay.sh "${VLLM_ROOT}"
build_stage "building exact CoreX GDN q/k map extension"
bash ./build_corex_gdn_qk_map.sh "${VLLM_ROOT}"
build_stage "building packed CoreX GDN decode extension"
bash ./build_corex_gdn_packed_decode.sh "${VLLM_ROOT}"
build_stage "building exact CoreX attention head RMSNorm extension"
bash ./build_corex_attn_head_rms_norm.sh "${VLLM_ROOT}"
