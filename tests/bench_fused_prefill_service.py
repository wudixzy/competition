...
def _post_stream(
        ...
        for raw_line in response:
            ...
            if ttft_s is None:
                ...
                if delta.get("content"):
                    ...
                    first_output_kind = "reasoning_content"
                else:
                    first_output_kind = "tool_calls"
    ...
    if first_output_kind == "content":
        first_token_ids = tokenizer.encode(
            normalized_output["content"], add_special_tokens=False)
        first_output_identity: Any = {
            "kind": first_output_kind,
            "token_id": first_token_ids[0] if first_token_ids else None,
        }
    elif first_output_kind == "reasoning_content":
        first_token_ids = tokenizer.encode(
            normalized_output["reasoning_content"],
            add_special_tokens=False)
        first_output_identity = {
            "kind": first_output_kind,
            "token_id": first_token_ids[0] if first_token_ids else None,
        }
    else:
        first_output_identity = {
            "kind": first_output_kind,
            "first_tool_delta": tool_deltas[0] if tool_deltas else None,
        }
    if (
        first_output_identity["kind"] is None
        or (
            first_output_identity["kind"] != "tool_calls"
            and first_output_identity["token_id"] is None
        )
    ):
        raise RuntimeError("stream completed without a first output token")
    first_token_sha256 = hashlib.sha256(json.dumps(
        first_output_identity, ensure_ascii=False, sort_keys=True,
    ...
    return {
        ...
        "finish_reason": finish_reason,
        "first_token_sha256": first_token_sha256,
...
def main() -> int:
    ...
    for target in targets:
        ...
        requests = [
            _post_stream(args.base, payload, args.timeout_s, tokenizer)
            for _ in range(3)
        ]
