...
def main() -> int:
    ...
    parser.add_argument("--grid-x", default="8,16,32,64,128")
    ...
    candidate = load_extension(
        "corex_moe_weight_gather_vec", args.candidate_extension)
    reducer = load_extension("corex_moe_exact_reduce", args.reduce_extension)
    grid_values = [int(value) for value in args.grid_x.split(",")]
    ...
    for grid_x in grid_values:
        selected_w13, selected_w2 = candidate.gather(
            w13, w2, ids, grid_x)
        actual = finish(selected_w13, selected_w2, weights)
        gather = measure(
            lambda gx=grid_x: candidate.gather(w13, w2, ids, gx),
            args.warmup, args.iterations, args.repeats)
        full = measure(
            lambda gx=grid_x: finish(
                *candidate.gather(w13, w2, ids, gx), weights),
            args.warmup, args.iterations, args.repeats)
        gather["speedup_vs_baseline"] = (
            results["baseline_gather"]["median_ms"] / gather["median_ms"])
        full["speedup_vs_baseline"] = (
            results["baseline_full"]["median_ms"] / full["median_ms"])
        results["grid"][str(grid_x)] = {
    ...
    best_grid = min(
        grid_values,
        key=lambda value: results["grid"][str(value)]["full"]["median_ms"],
    )
    ...
    def candidate_routed():
        routed_weights, routed_ids = route(router_logits)
        return finish(
            *candidate.gather(w13, w2, routed_ids, best_grid), routed_weights)
    ...
    for _ in range(args.sequence_steps):
        ...
        actual = finish(
            *candidate.gather(w13, w2, step_ids, best_grid), step_weights)
