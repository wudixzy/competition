...
PATCH_SCRIPT = SCRIPTS / "patch_cuda_graph.py"
...
class CudaGraphPatchUnitTest(unittest.TestCase):

    def test_patch_is_idempotent(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = pathlib.Path(tmp)
            target = _make_fake_vllm(
                root, "config = ModelConfig(\n            enforce_eager=True,\n)\n")
    ...
    def test_patch_fails_for_unknown_layout(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = pathlib.Path(tmp)
            _make_fake_vllm(root, "config = ModelConfig()\n")
