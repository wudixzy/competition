...
def main() -> int:
    ...
    norm_weight = torch.randn(
        ...
        dtype=torch.float16) * 0.05
    a_log = torch.zeros(
        (local_value_heads,), device=device, dtype=torch.float32)
    dt_bias = torch.zeros(
        (local_value_heads,), device=device, dtype=torch.float32)
    ...
    decay = (-a_log.exp() * F.softplus(a_all.float() + dt_bias)).unsqueeze(1)
    ...
    def beta_decay_stage() -> torch.Tensor:
        b_all.sigmoid()
        current_decay = -a_log.exp() * F.softplus(a_all.float() + dt_bias)
    ...
    full_conv_state = initial_conv_state.clone()
    full_temporal_state = initial_temporal_state.clone()

    def local_full_decode() -> torch.Tensor:
        ...
        current_k = (current_k * torch.rsqrt(
            ...
            + 1e-6)).float()
        current_g = (-a_log.exp() * F.softplus(
            current_a.float() + dt_bias)).float().exp()
        current_beta = current_b.sigmoid().float()
        full_temporal_state.mul_(current_g[:, :, None, None])
        flat = full_temporal_state.view(-1, head_dim, head_dim)
        bh = flat.shape[0]
        memory = torch.bmm(
    ...
    cases = {
        ...
        "beta_decay": beta_decay_stage,
        "qk_prep": qk_prep_stage,
        "recurrent": recurrent_stage,
        "gated_norm": gated_norm_stage,
        "output_projection": output_projection,
        "local_full_decode": local_full_decode,
    }
    timings = {
    ...
    isolated_sum = sum(
        result["median_ms"] for name, result in timings.items()
        if name != "local_full_decode")
    for name, result in timings.items():
        if name != "local_full_decode":
            result["isolated_share_pct"] = (
                result["median_ms"] / isolated_sum * 100.0)

    output = local_full_decode()
    torch.cuda.synchronize()
    report = {
        ...
        "composition_overhead_ms": (
            timings["local_full_decode"]["median_ms"] - isolated_sum),
        "timings": timings,
    }
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(report, indent=2) + "\n")
    print(json.dumps(report, indent=2))
    return 0 if report["checks"]["output_finite"] else 1
