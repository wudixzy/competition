...
OUTPUT_DIR=${1:?usage: build_corex_block_major_kv_transfer.sh OUTPUT_DIR}
COREX_ROOT=${COREX_ROOT:-/usr/local/corex-3.2.3}
TORCH_ROOT=${TORCH_ROOT:-${COREX_ROOT}/lib64/python3/dist-packages/torch}
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUTPUT=${OUTPUT_DIR}/corex_block_major_kv_transfer.so
...
"${COREX_ROOT}/bin/clang++" \
    ...
    "${ROOT}/qwen3_6_scripts/corex_block_major_kv_transfer.cu" \
