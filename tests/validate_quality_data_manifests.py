...
def validate_matrix(value: Any) -> list[str]:
    ...
    if (value.get("schema") != "bi100-long-context-quality-matrix-v6"
    ...
    expected_cache_trace = {
        ...
        "admission64_first_sibling_must_effectively_miss": True,
        "admission64_repeated_branch_admission_required": True,
        "admission64_subsequent_sibling_must_restore": True,
    ...
    if (reasoning.get("validation") != (
            "the expected marker sequence and arithmetic answer occur exactly "
            "once as the final content suffix, with nonempty separated "
            "reasoning and a natural stop within a 1024-token output budget; "
            "strict instruction following is gated independently")):
        reasons.append("131K reasoning semantic validation contract differs")
    expected_partial_contract = {
        "32k_partial_branch": {
            "request_shape": "three_sibling_branch_text",
            "cache_scenario": "policy_aware_partial_shared_prefix",
            "equivalence": "branch_rules_cache_trace_and_baseline_exact",
            "validation": (
                "A cold/warm exact; B and C markers isolated; fine32 B/C "
                "strict-partial hits; admission64 B effective miss with "
                "repeated-branch admission followed by C strict-partial "
                "restore"),
        },
        "235k_partial_branch": {
            "request_shape": "three_sibling_branch_text",
            "cache_scenario": "policy_aware_partial_shared_prefix",
            "equivalence": "cache_trace_warm_repeat_and_baseline_semantic",
            "validation": (
                "A cold/warm exact; B and C markers isolated; policy-aware "
                "partial restore; deterministic B warm repeat"),
        },
    }
    for case_id, expected_contract in expected_partial_contract.items():
        partial = case_map.get(case_id) or {}
        if any(partial.get(field) != expected
               for field, expected in expected_contract.items()):
            reasons.append(f"{case_id} policy-aware cache contract differs")
    serialized = json.dumps(value, ensure_ascii=True).lower()
...
def main() -> int:
    ...
    parser.add_argument(
        "--matrix", type=Path,
        default=ROOT / "quality/long_context_matrix.v6.json")
    parser.add_argument(
