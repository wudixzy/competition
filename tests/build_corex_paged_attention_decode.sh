...
OUTPUT_DIR=${1:?usage: build_corex_paged_attention_decode.sh OUTPUT_DIR}
COREX_ROOT=${COREX_ROOT:-/usr/local/corex-3.2.3}
TORCH_ROOT=${TORCH_ROOT:-${COREX_ROOT}/lib64/python3/dist-packages/torch}
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT=${OUTPUT_DIR}/corex_paged_attention_decode.so
...
"${COREX_ROOT}/bin/clang++" \
    ...
    -DTORCH_EXTENSION_NAME=corex_paged_attention_decode \
    ...
    "${SCRIPT_DIR}/corex_paged_attention_decode_ext.cu" \
...
test -s "${OUTPUT}"
printf '[ok] CoreX direct paged-attention extension %s\n' "${OUTPUT}"
