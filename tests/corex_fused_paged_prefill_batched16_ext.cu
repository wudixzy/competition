...
#if defined(BI100_PREFILL_BATCHED16_EXPERIMENT)
constexpr int kMaxBatchedGemmCount = kSplitCount * kNumQueryHeads;
#endif
...
__global__ void build_split_head_pointer_arrays(
    const float* matrix_a, const float* matrix_b, float* matrix_c,
    unsigned long long* pointer_workspace, int active_splits,
    int64_t a_split_stride, int64_t a_head_stride,
    int64_t b_split_stride, int64_t b_head_stride,
    int64_t c_split_stride, int64_t c_head_stride) {
  const int index = threadIdx.x;
  const int count = active_splits * kNumQueryHeads;
  if (index >= count) {
    return;
  }
  const int split = index / kNumQueryHeads;
  const int head = index % kNumQueryHeads;
  auto** a_ptrs = reinterpret_cast<const float**>(pointer_workspace);
  auto** b_ptrs = reinterpret_cast<const float**>(
      pointer_workspace + kMaxBatchedGemmCount);
  auto** c_ptrs = reinterpret_cast<float**>(
      pointer_workspace + 2 * kMaxBatchedGemmCount);
  a_ptrs[index] =
      matrix_a + split * a_split_stride + head * a_head_stride;
  b_ptrs[index] =
      matrix_b + split * b_split_stride + head * b_head_stride;
  c_ptrs[index] =
      matrix_c + split * c_split_stride + head * c_head_stride;
}
#endif
...
#if defined(BI100_PREFILL_BATCHED16_EXPERIMENT)
cublasStatus_t qk_split_head_batched(
    cublasHandle_t handle, unsigned long long* pointer_workspace,
    int query_len, int batch_count) {
  const float alpha = 1.0f;
  const float beta = 0.0f;
  auto** a_ptrs = reinterpret_cast<const float**>(pointer_workspace);
  auto** b_ptrs = reinterpret_cast<const float**>(
  ...
  return cublasSgemmBatched(
      handle, CUBLAS_OP_T, CUBLAS_OP_N,
      kTileTokens, query_len, kHeadDim,
      &alpha, reinterpret_cast<const float* const*>(a_ptrs), kHeadDim,
      reinterpret_cast<const float* const*>(b_ptrs), kHeadDim,
      &beta, reinterpret_cast<float* const*>(c_ptrs), kTileTokens,
      batch_count);
}
...
cublasStatus_t pv_split_head_batched(
  ...
  return cublasSgemmBatched(
      ...
      kHeadDim, query_len, kTileTokens,
      &alpha, reinterpret_cast<const float* const*>(a_ptrs), kHeadDim,
      reinterpret_cast<const float* const*>(b_ptrs), kTileTokens,
      &beta, reinterpret_cast<float* const*>(c_ptrs), kHeadDim,
      batch_count);
}
#endif
...
#if defined(BI100_PREFILL_BATCHED16_EXPERIMENT)
  auto pointer_workspace = torch::empty(
      {3 * kMaxBatchedGemmCount},
  ...
  const auto run_group = [&](int group_start, int group_tokens,
...
#if defined(BI100_PREFILL_BATCHED16_EXPERIMENT)
    const int batch_count = active_splits * kNumQueryHeads;
    build_split_head_pointer_arrays<<<1, 32, 0, stream>>>(
        key_tiles.data_ptr<float>(), converted_query.data_ptr<float>(),
        scores.data_ptr<float>(),
        reinterpret_cast<unsigned long long*>(
            pointer_workspace.data_ptr<int64_t>()),
        active_splits,
        key_split_stride, 0,
        0, static_cast<int64_t>(query_len) * kHeadDim,
        score_split_stride,
        static_cast<int64_t>(query_len) * kTileTokens);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    check_cublas(qk_split_head_batched(
        handle,
        reinterpret_cast<unsigned long long*>(
            pointer_workspace.data_ptr<int64_t>()),
        query_len, batch_count),
        "split4 pointer-batched paged prefill QK");
#else
    for (int split = 0; split < active_splits; ++split) {
...
#if defined(BI100_PREFILL_BATCHED16_EXPERIMENT)
    build_split_head_pointer_arrays<<<1, 32, 0, stream>>>(
        value_tiles.data_ptr<float>(), scores.data_ptr<float>(),
        split_output.data_ptr<float>(),
        reinterpret_cast<unsigned long long*>(
            pointer_workspace.data_ptr<int64_t>()),
        active_splits,
        key_split_stride, 0,
        ...
        static_cast<int64_t>(query_len) * kTileTokens,
        output_split_stride,
        static_cast<int64_t>(query_len) * kHeadDim);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    check_cublas(pv_split_head_batched(
        ...
        reinterpret_cast<unsigned long long*>(
            pointer_workspace.data_ptr<int64_t>()),
        query_len, batch_count),
        "split4 pointer-batched paged prefill PV");
