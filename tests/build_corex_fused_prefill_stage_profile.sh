...
OUTPUT_DIR=${1:?usage: build_corex_fused_prefill_stage_profile.sh OUTPUT_DIR}
COREX_ROOT=${COREX_ROOT:-/usr/local/corex-3.2.3}
TORCH_ROOT=${TORCH_ROOT:-${COREX_ROOT}/lib64/python3/dist-packages/torch}
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT=${OUTPUT_DIR}/corex_fused_paged_prefill_profile.so
...
"${COREX_ROOT}/bin/clang++" \
    ...
    "${SCRIPT_DIR}/corex_fused_paged_prefill_stage_profile_ext.cu" \
...
test -s "${OUTPUT}"
printf '[ok] CoreX fused-prefill stage profiler %s\n' "${OUTPUT}"
