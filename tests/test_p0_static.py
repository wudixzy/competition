...
class P0StaticCoverageTest(unittest.TestCase):
    ...
    def test_bi100_preflight_checks_each_gpu_with_timeout(self):
        ...
        self.assertIn("os.killpg(process.pid, signal.SIGTERM)", src)
        self.assertIn("TERM_GRACE_S = 60.0", src)
        self.assertIn("os.killpg(process.pid, signal.SIGKILL)", src)
        self.assertIn("process.communicate(", src)
    ...
    def test_bi100_nccl_preflight_checks_timed_all_reduce(self):
        ...
        self.assertIn("signal.SIG_UNBLOCK", src)
        self.assertIn("os.setsid()", src)
        self.assertIn("os.killpg(process.pid, signum)", src)
        self.assertIn("TERM_GRACE_S = 60.0", src)
        self.assertIn("KILL_GRACE_S = 20.0", src)
        self.assertIn("_cleanup_active_processes()", src)
