...
"${COREX_ROOT}/bin/clang++" \
    ...
    -DTORCH_EXTENSION_NAME=corex_gdn_qkv_map \
    ...
    "${SCRIPT_DIR}/corex_gdn_qkv_map_ext.cu" \
...
test -s "${OUTPUT}"
printf '[ok] CoreX exact GDN q/k/v map probe %s\n' "${OUTPUT}"
