...
"${COREX_ROOT}/bin/clang++" \
    ...
    -DTORCH_EXTENSION_NAME=corex_ixinfer_fmha_probe \
    ...
    "${SCRIPT_DIR}/corex_ixinfer_fmha_probe_ext.cu" \
    ...
    -lc10_cuda -lc10 -lcuinfer -lcudart -o "${OUTPUT}"

test -s "${OUTPUT}"
printf '[ok] CoreX ixinfer FMHA probe %s\n' "${OUTPUT}"
