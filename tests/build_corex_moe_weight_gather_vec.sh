...
"${COREX_ROOT}/bin/clang++" \
    ...
    -DTORCH_EXTENSION_NAME=corex_moe_weight_gather_vec \
    ...
    "${SCRIPT_DIR}/corex_moe_weight_gather_vec_ext.cu" \
...
test -s "${OUTPUT}"
printf '[ok] CoreX vector MoE selected-weight gather %s\n' "${OUTPUT}"
