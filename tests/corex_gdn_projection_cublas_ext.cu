...
torch::Tensor linear_cublas(const torch::Tensor& input,
  ...
  if (mode == -2) {
    ...
    status = cublasHgemm(
        ...
        output_size, 1, input_size, &alpha,
        reinterpret_cast<const half*>(weight.data_ptr<at::Half>()),
        input_size,
        reinterpret_cast<const half*>(input.data_ptr<at::Half>()),
        input_size, &beta,
        reinterpret_cast<half*>(output.data_ptr<at::Half>()), output_size);
  } else {
    ...
    status = cublasGemmEx(
        ...
        &beta, output.data_ptr<at::Half>(), CUDA_R_16F, output_size,
        CUDA_R_32F, static_cast<cublasGemmAlgo_t>(mode));
  }
  TORCH_CHECK(status == CUBLAS_STATUS_SUCCESS,
              "cuBLAS mode ", mode, " failed with status ",
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def("linear", &linear_cublas,
             "FP16 GDN projection cuBLAS algorithm probe");
}
