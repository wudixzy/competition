...
__global__ void qk_map_kernel(const half* query, const half* key,
  ...
  const int source =
      ((batch_index * key_heads + key_head) * kHeadDim + dim);
  output[index] = __half2float(query[source]) * kQueryScale;
  output[elements + index] = __half2float(key[source]);
}

__global__ void qkv_map_kernel(const half* query, const half* key,
                               const half* value, float* output, int batch,
  ...
  output[index] = __half2float(query[source]) * kQueryScale;
  output[elements + index] = __half2float(key[source]);
  output[2 * elements + index] = __half2float(value[index]);
}
...
void check_input(const torch::Tensor& tensor, const char* name) {
  ...
  TORCH_CHECK(tensor.dim() == 3 && tensor.size(2) == kHeadDim,
              name, " must have shape (batch, heads, 128)");
}

void check_qk(const torch::Tensor& query, const torch::Tensor& key,
              int value_heads) {
...
torch::Tensor qk_map(const torch::Tensor& query,
  ...
  check_qk(query, key, value_heads);
...
torch::Tensor qkv_map(const torch::Tensor& query,
                      const torch::Tensor& key,
                      const torch::Tensor& value) {
  check_input(value, "value");
  const int batch = static_cast<int>(query.size(0));
  const int value_heads = static_cast<int>(value.size(1));
  check_qk(query, key, value_heads);
  TORCH_CHECK(value.size(0) == batch,
              "query and value batch dimensions must match");
  const int key_heads = static_cast<int>(query.size(1));
  torch::Tensor output = torch::empty(
      {3, batch, value_heads, kHeadDim},
      query.options().dtype(torch::kFloat32));
  const int elements = batch * value_heads * kHeadDim;
  constexpr int threads = 256;
  const int blocks = (elements + threads - 1) / threads;
  qkv_map_kernel<<<blocks, threads, 0,
                   at::cuda::getCurrentCUDAStream()>>>(
      reinterpret_cast<const half*>(query.data_ptr<at::Half>()),
      reinterpret_cast<const half*>(key.data_ptr<at::Half>()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  ...
  module.def("qkv_map", &qkv_map,
             "Map normalized FP16 q/k and cast FP16 value to FP32");
}
