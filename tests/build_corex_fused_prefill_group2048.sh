...
OUTPUT_DIR=${1:?usage: build_corex_fused_prefill_group2048.sh OUTPUT_DIR}
COREX_ROOT=${COREX_ROOT:-/usr/local/corex-3.2.3}
TORCH_ROOT=${TORCH_ROOT:-${COREX_ROOT}/lib64/python3/dist-packages/torch}
ROOT=$(cd "$(dirname "$0")/.." && pwd)
OUTPUT=${OUTPUT_DIR}/corex_fused_paged_prefill_group2048.so
...
"${COREX_ROOT}/bin/clang++" \
    ...
    "$ROOT/tests/corex_fused_paged_prefill_group2048_ext.cu" \
...
test -s "$OUTPUT"
printf '[ok] CoreX group2048 fused paged-prefill extension %s\n' "$OUTPUT"
