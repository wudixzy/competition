...
OUTPUT_DIR=${1:?usage: build_corex_fused_prefill_grouped_qk_split512.sh OUTPUT_DIR}
COREX_ROOT=${COREX_ROOT:-/usr/local/corex-3.2.3}
TORCH_ROOT=${TORCH_ROOT:-${COREX_ROOT}/lib64/python3/dist-packages/torch}
ROOT=$(cd "$(dirname "$0")/.." && pwd)
OUTPUT=${OUTPUT_DIR}/corex_fused_paged_prefill_grouped_qk_split512.so
...
test -s "$OUTPUT"
printf '[ok] CoreX grouped-QK split512 paged-prefill extension %s\n' "$OUTPUT"
