#!/usr/bin/env python3
"""Verify immutable M1-68 baseline/candidate overlays for M1-72."""
...
Json = dict[str, Any]
SCHEMA = "bi100-m1-72-runtime-pair-v1"
VERSION = 1
ALLOWED_RUNTIME_FILE_DELTA = {"api_server", "chat_utils", "protocol"}
...
def _validate_overlay(
    ...
    if (install.get("versions") or {}).get("transformers") != "4.55.3":
        reasons.append(f"{label} Transformers version mismatch")
...
def verify(
    ...
    observed_delta = {
        ...
        if control["file_sha256"].get(name)
        != candidate["file_sha256"].get(name)
    }
    if observed_delta != ALLOWED_RUNTIME_FILE_DELTA:
        reasons.append(
            "runtime file delta differs from api_server/chat_utils/protocol: "
