...
OUTPUT_DIR=${1:?usage: build_corex_attention_wmma_qk_probe.sh OUTPUT_DIR}
COREX_ROOT=${COREX_ROOT:-/usr/local/corex-3.2.3}
TORCH_ROOT=${TORCH_ROOT:-${COREX_ROOT}/lib64/python3/dist-packages/torch}
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT=${OUTPUT_DIR}/corex_attention_wmma_qk_probe.so
...
"${COREX_ROOT}/bin/clang++" \
    ...
    -DTORCH_EXTENSION_NAME=corex_attention_wmma_qk_probe \
    ...
    "${SCRIPT_DIR}/corex_attention_wmma_qk_probe.cu" \
...
test -s "${OUTPUT}"
printf '[ok] CoreX WMMA QK probe %s\n' "${OUTPUT}"
