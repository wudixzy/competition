...
constexpr int kGroupTokens = 2048;
...
__global__ void gather_kv_group_kernel(
  ...
  const int64_t total = static_cast<int64_t>(group_tokens) * kHeadDim;
  for (int64_t index =
           static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
       index < total;
       index += static_cast<int64_t>(blockDim.x) * gridDim.x) {
    const int token_offset = index / kHeadDim;
    const int dim = index - static_cast<int64_t>(token_offset) * kHeadDim;
    const int logical_token = group_start + token_offset;
...
__global__ void mask_group_scores_kernel(
    ...
    int group_start, int group_tokens, int rows) {
  const int64_t elements = static_cast<int64_t>(rows) * group_tokens;
  for (int64_t index =
           static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
       index < elements;
       index += static_cast<int64_t>(blockDim.x) * gridDim.x) {
    const int row = index / group_tokens;
    const int column = index - static_cast<int64_t>(row) * group_tokens;
    const int query_index = row % query_len;
    const int logical_token = group_start + column;
    if (logical_token > context_len + query_index) {
...
__global__ void normalize_group_scores_kernel(
    float* scores, float* correction, float* running_max,
    float* running_sum, int group_tokens, int rows) {
  ...
  __shared__ float group_correction;
  ...
  for (int column = threadIdx.x; column < group_tokens;
       column += blockDim.x) {
  ...
  if (threadIdx.x == 0) {
    ...
    correction[row] = group_correction;
  }
  __syncthreads();
  ...
  if (threadIdx.x == 0) {
    ...
    running_sum[row] = __fadd_rn(
        __fmul_rn(state_sum, group_correction), reduction[0]);
  }
}
...
cublasStatus_t qk_group(
    cublasHandle_t handle, const float* key_tile, const float* query,
    float* scores, int group_tokens, int rows) {
...
std::vector<torch::Tensor> fused_paged_prefill_forward(
  ...
  auto key_tiles = torch::empty(
      {kGroupTokens, kHeadDim}, float_options);
  auto value_tiles = torch::empty(
      {kGroupTokens, kHeadDim}, float_options);
  auto scores = torch::empty(
      {kNumQueryHeads, query_len, kGroupTokens}, float_options);
  ...
  const auto run_group = [&](int group_start, int group_tokens,
                             bool causal) {
    TORCH_CHECK(group_tokens > 0 && group_tokens <= kGroupTokens,
                "invalid paged-prefill group length");
    ...
    check_cublas(qk_group(
        handle, key_tiles.data_ptr<float>(),
        converted_query.data_ptr<float>(), scores.data_ptr<float>(),
        group_tokens, rows), "group2048 paged prefill QK");

    if (causal) {
      const int64_t score_elements =
          static_cast<int64_t>(rows) * group_tokens;
    ...
    normalize_group_scores_kernel<<<rows, kThreads, 0, stream>>>(
        scores.data_ptr<float>(), correction.data_ptr<float>(),
        running_max.data_ptr<float>(), running_sum.data_ptr<float>(),
        group_tokens, rows);
    C10_CUDA_KERNEL_LAUNCH_CHECK();

    check_cublas(pv_group(
        handle, value_tiles.data_ptr<float>(), scores.data_ptr<float>(),
        tile_output.data_ptr<float>(), group_tokens, rows),
        "group2048 paged prefill PV");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def("forward", &fused_paged_prefill_forward,
             "FP32 paged-prefill pipeline with flattened GQA group GEMMs");
}
