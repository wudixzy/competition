...
void check_cuda(cudaError_t status, const char* operation) {
  TORCH_CHECK(status == cudaSuccess, operation, " failed: ",
              cudaGetErrorString(status));
}

class StageRecorder {
 public:
  StageRecorder(bool enabled, cudaStream_t stream)
  ...
  ~StageRecorder() {
    for (cudaEvent_t event : boundaries_) {
  ...
  void finish_stage(ProfileStage stage) {
    if (!enabled_) {
      return;
    }
    stages_.push_back(static_cast<int>(stage));
    record_boundary();
  }

  std::vector<double> finish(double validation_host_ms) {
    if (!enabled_) {
      return {};
    }
    TORCH_CHECK(boundaries_.size() == stages_.size() + 1,
                "invalid profile boundary sequence");
    check_cuda(cudaEventSynchronize(boundaries_.back()),
               "profile event synchronization");
    std::vector<double> values(kProfileStageCount + 3, 0.0);
    for (size_t index = 0; index < stages_.size(); ++index) {
      float elapsed_ms = 0.0f;
      check_cuda(
          cudaEventElapsedTime(
              &elapsed_ms, boundaries_[index], boundaries_[index + 1]),
          "profile event elapsed time");
      values[stages_[index]] += static_cast<double>(elapsed_ms);
      values[kProfileStageCount] += static_cast<double>(elapsed_ms);
    }
    values[kProfileStageCount + 1] = validation_host_ms;
    values[kProfileStageCount + 2] =
 ...
 private:
  void record_boundary() {
    ...
    boundaries_.push_back(event);
    check_cuda(cudaEventRecord(event, stream_), "profile event record");
  }

  bool enabled_;
  cudaStream_t stream_;
  std::vector<cudaEvent_t> boundaries_;
...
std::vector<torch::Tensor> fused_paged_prefill_forward_impl(
    ...
    int64_t context_len_arg, double scale_arg, bool record_stages) {
  ...
  const double validation_host_ms =
  ...
  StageRecorder recorder(record_stages, stream);
  recorder.start();
  ...
  recorder.finish_stage(ProfileStage::kInit);
  ...
  const auto run_group = [&](int group_start, int group_tokens,
    ...
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    recorder.finish_stage(ProfileStage::kGather);
    ...
    recorder.finish_stage(ProfileStage::kQk);
    ...
    recorder.finish_stage(ProfileStage::kMask);
    ...
    normalize_split_scores_kernel<<<rows, kThreads, 0, stream>>>(
        ...
        active_splits, rows);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    recorder.finish_stage(ProfileStage::kSoftmax);
    ...
    recorder.finish_stage(ProfileStage::kPv);
    ...
    merge_split_output_kernel<<<
        ...
        output_elements);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    recorder.finish_stage(ProfileStage::kMerge);
  };
  ...
  recorder.finish_stage(ProfileStage::kFinalize);
  if (!record_stages) {
    return {output, lse};
  }
  auto timing = torch::tensor(
      recorder.finish(validation_host_ms),
      torch::TensorOptions().dtype(torch::kFloat64).device(torch::kCPU));
  return {output, lse, timing};
}
...
std::vector<torch::Tensor> fused_paged_prefill_forward(
  ...
  return fused_paged_prefill_forward_impl(
      query, key_new, value_new, key_cache, value_cache, block_table,
      context_len_arg, scale_arg, false);
}

std::vector<torch::Tensor> fused_paged_prefill_profile(
    const torch::Tensor& query, const torch::Tensor& key_new,
    const torch::Tensor& value_new, const torch::Tensor& key_cache,
    const torch::Tensor& value_cache, const torch::Tensor& block_table,
    int64_t context_len_arg, double scale_arg) {
  return fused_paged_prefill_forward_impl(
      query, key_new, value_new, key_cache, value_cache, block_table,
      context_len_arg, scale_arg, true);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def("forward", &fused_paged_prefill_forward,
             "Fixed-shape FP32 paged-prefill pipeline for cache-only context");
  module.def("profile", &fused_paged_prefill_profile,
             "Profile fixed-shape FP32 paged-prefill pipeline stages");
}
