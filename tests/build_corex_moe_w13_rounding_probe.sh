...
OUTPUT_DIR=${1:?usage: build_corex_moe_w13_rounding_probe.sh OUTPUT_DIR}
COREX_ROOT=${COREX_ROOT:-/usr/local/corex-3.2.3}
TORCH_ROOT=${TORCH_ROOT:-${COREX_ROOT}/lib64/python3/dist-packages/torch}
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT=${OUTPUT_DIR}/corex_moe_w13_rounding_probe.so
...
"${COREX_ROOT}/bin/clang++" \
    ...
    -DTORCH_EXTENSION_NAME=corex_moe_w13_rounding_probe \
    ...
    "${SCRIPT_DIR}/corex_moe_w13_rounding_probe_ext.cu" \
...
test -s "${OUTPUT}"
printf '[ok] CoreX W13 rounding probe %s\n' "${OUTPUT}"
