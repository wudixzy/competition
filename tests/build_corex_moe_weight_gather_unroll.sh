...
OUTPUT_DIR=${1:?usage: build_corex_moe_weight_gather_unroll.sh OUTPUT_DIR}
COREX_ROOT=${COREX_ROOT:-/usr/local/corex-3.2.3}
TORCH_ROOT=${TORCH_ROOT:-${COREX_ROOT}/lib64/python3/dist-packages/torch}
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT=${OUTPUT_DIR}/corex_moe_weight_gather_unroll.so
...
"${COREX_ROOT}/bin/clang++" \
    ...
    -DTORCH_EXTENSION_NAME=corex_moe_weight_gather_unroll \
    ...
    "${SCRIPT_DIR}/corex_moe_weight_gather_unroll_ext.cu" \
...
test -s "${OUTPUT}"
printf '[ok] CoreX packed/unrolled MoE gather %s\n' "${OUTPUT}"
