...
OUTPUT_DIR=${1:?usage: build_m1_111_query_tiled_retest.sh OUTPUT_DIR}
COREX_ROOT=${COREX_ROOT:-/usr/local/corex-3.2.3}
TORCH_ROOT=${TORCH_ROOT:-${COREX_ROOT}/lib64/python3/dist-packages/torch}
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUTPUT=${OUTPUT_DIR}/corex_query_tiled_paged_prefill_retest.so
...
"${COREX_ROOT}/bin/clang++" \
    ...
    -DTORCH_EXTENSION_NAME=corex_query_tiled_paged_prefill_retest \
...
test -s "${OUTPUT}"
printf '[ok] CoreX query-tiled paged-prefill extension %s\n' "${OUTPUT}"
