...
__global__ void convert_query_kernel(const __half* query, float* converted,
  ...
  for (int64_t index =
    ...
    const int query_index = (index / kHeadDim) % query_len;
    const int head =
...
__global__ void gather_kv_group_kernel(
    ...
    int padded_tokens) {
  const int64_t total = static_cast<int64_t>(padded_tokens) * kHeadDim;
  for (int64_t index =
           static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
       index < total;
       index += static_cast<int64_t>(blockDim.x) * gridDim.x) {
    ...
    if (token_offset >= group_tokens) {
      // Fixed 512-token softmax/PV splits require zero-filled tail columns.
...
__global__ void mask_group_scores_kernel(
    ...
    int group_start, int group_tokens, int padded_tokens,
    int rows, bool causal) {
  const int64_t elements = static_cast<int64_t>(rows) * padded_tokens;
  for (int64_t index =
           static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
       index < elements;
       index += static_cast<int64_t>(blockDim.x) * gridDim.x) {
    const int row = index / padded_tokens;
    const int column = index - static_cast<int64_t>(row) * padded_tokens;
    const int query_index = row % query_len;
    const int logical_token = group_start + column;
    if (column >= group_tokens
...
cublasStatus_t qk_group(
    ...
    float* scores, int padded_tokens, int rows) {
...
std::vector<torch::Tensor> fused_paged_prefill_forward(
  ...
  const int64_t value_split_stride =
  ...
  const auto run_group = [&](int group_start, int group_tokens,
    ...
    gather_kv_group_kernel<<<kGatherBlocks, kThreads, 0, stream>>>(
        ...
        group_tokens, padded_tokens);
    C10_CUDA_KERNEL_LAUNCH_CHECK();

    check_cublas(qk_group(
        handle, key_tiles.data_ptr<float>(),
        converted_query.data_ptr<float>(), scores.data_ptr<float>(),
        padded_tokens, rows), "grouped-QK split512 paged prefill QK");

    const bool needs_mask = causal || group_tokens != padded_tokens;
    if (needs_mask) {
      const int64_t score_elements =
          static_cast<int64_t>(rows) * padded_tokens;
      mask_group_scores_kernel<<<
          ...
          group_tokens, padded_tokens, rows, causal);
      C10_CUDA_KERNEL_LAUNCH_CHECK();
    }
    ...
    for (int split = 0; split < active_splits; ++split) {
      check_cublas(pv_split(
          handle,
          value_tiles.data_ptr<float>() + split * value_split_stride,
          scores.data_ptr<float>() + split * kTileTokens,
          split_output.data_ptr<float>() + split * output_split_stride,
          rows), "grouped-QK split512 paged prefill PV");
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  module.def(
      "forward", &fused_paged_prefill_forward,
      "Grouped GQA QK with fixed-512 FP32 softmax/PV boundaries");
}
