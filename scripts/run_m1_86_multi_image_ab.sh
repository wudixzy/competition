...
PY
                    ...
                    read -r observed_pgid observed_token \
                        <<< "$observed_identity"
        ...
        if [[ "$process_group_ok" != 1 \
            ...
            if python3 "$ROOT/scripts/wait_http_health.py" \
                    ...
                    --out "$arm/startup.json" \
                    > "$arm/startup.stdout" 2> "$arm/startup.stderr"; then
                startup_ok=1
            fi
        fi
    fi
...
python3 "$ROOT/tests/compare_m1_86_multi_image_ab.py" \
    ...
    --control-startup "$RUN_ROOT/control/startup.json" \
    --candidate-startup "$RUN_ROOT/candidate/startup.json" \
    ...
    --candidate-postflight "$RUN_ROOT/candidate/service_postflight.json" \
    --control-preflight-comparison \
        "$RUN_ROOT/control/preflight_comparison.json" \
    --candidate-preflight-comparison \
        "$RUN_ROOT/candidate/preflight_comparison.json" \
