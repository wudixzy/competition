...
MATRIX_DIR=${M1_33_MATRIX_DIR:-$RUN_ROOT/admission64_chunk64_fixed}
OUTPUT_DIR=${M1_33_POST_DIR:-$RUN_ROOT/admission64_chunk64_post}
...
run_gate() {
    ...
    if [[ $rc -ne 0 ]]; then
        echo "M1-33 post-matrix gate failed: $name rc=$rc" >&2
        return "$rc"
    fi
    health
}
...
run_gate long_131k_exact 5400 \
    ...
    --min-cached-tokens 130944 --equivalence-mode exact \
    --timeout-s 2400 --run-id m1_33_chunk64_131k \
...
run_gate long_262k_capacity 9000 \
    ...
    --min-cached-tokens 261952 --equivalence-mode exact \
    --timeout-s 4200 --run-id m1_33_chunk64_262k \
...
echo "M1-33 post-matrix long-context and 256K capacity gates passed"
