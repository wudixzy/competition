...
mkdir -p "$RUN_ROOT/long_131k_exact"
...
run_gate long_262k_safe 60 \
    ...
    --target-prompt-tokens 262000 --max-tokens 16 \
    --min-cached-tokens 261984 --min-completion-tokens 16 \
    --equivalence-mode exact
...
python3 "$ROOT/tests/qualify_m1_49_long_context.py" \
    ...
    --smoke "$RUN_ROOT/smoke.json" \
    --multimodal "$RUN_ROOT/multimodal_qualification.json" \
