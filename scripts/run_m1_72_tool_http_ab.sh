...
CONTROL_REVISION=${CONTROL_REVISION:-c78d55d0a7637baf4910af68b6d6ba4e286a1254}
CANDIDATE_REVISION=${CANDIDATE_REVISION:-cdb1bc41f728a5610a3632ad7923d73a90748919}
ACTIVE_PID=""
ACTIVE_PGID=""
...
if [[ ! "$PORT" =~ ^[0-9]+$ || "$PORT" -lt 1 || "$PORT" -gt 65534 ]]; then
    echo "PORT must be between 1 and 65534 for two isolated arms" >&2
    exit 2
fi
...
if [[ -n "$(git -C "$ROOT" status --porcelain --untracked-files=all -- \
    ...
    echo "M1-72 runner refuses a dirty source tree" >&2
    exit 3
fi
...
write_arm_status() {
    ...
    local object_status=$4
    local runtime_revision=$5
    local arm_port=$6
    python3 - "$arm" "$final_rc" "$strict_status" "$object_status" \
            "$runtime_revision" "$arm_port" <<'PY'
import hashlib
import json
from pathlib import Path
import sys
...
report = {
    "schema": "bi100-m1-72-tool-http-arm-v1",
    "version": 1,
    "qualified": int(sys.argv[2]) == 0,
    "runtime_revision": sys.argv[5],
    "port": int(sys.argv[6]),
    "strict_false_expected_status": int(sys.argv[3]),
    "object_history_expected_status": int(sys.argv[4]),
    "gates": {
...
run_arm() {
    ...
    local strict_status=$4
    local object_status=$5
    ...
    if [[ $rc -ne 0 ]]; then
        write_arm_status \
            "$arm" 1 "$strict_status" "$object_status" \
...
PY
    ...
    if [[ "$startup_ok" == 1 ]]; then
        ...
        timeout --signal=TERM --kill-after=70s 1800s \
            ...
            --timeout-s 300 \
            --strict-false-expected-status "$strict_status" \
            --object-history-expected-status "$object_status" \
    ...
    if [[ "$require_attribution" == 1 && -f "$arm/server.log" ]]; then
        ...
        attribution_rc=0
        printf '%s\n' \
            '{"schema":"baseline-attribution-not-compared","qualified":true}' \
    ...
    write_arm_status \
        "$arm" "$arm_rc" "$strict_status" "$object_status" \
        "$runtime_revision" "$arm_port"
    return "$arm_rc"
}
...
report = {
    "schema": "bi100-m1-72-tool-http-ab-runner-v1",
    "version": 1,
...
set +e
PYTHONPATH="$ROOT/tests" python3 "$ROOT/tests/verify_m1_72_runtime_pair.py" \
...
run_arm baseline "$CONTROL_RUNTIME_SITE_PACKAGES" \
    "$CONTROL_REVISION" 400 400 0 "$PORT"
rc=$?
set -e
printf '%s\n' "$rc" > "$RUN_ROOT/baseline.rc"
[[ $rc -eq 0 ]]

CURRENT_STAGE=candidate
set +e
run_arm candidate "$CANDIDATE_RUNTIME_SITE_PACKAGES" \
    "$CANDIDATE_REVISION" 200 200 1 "$((PORT + 1))"
rc=$?
set -e
...
set +e
python3 "$ROOT/tests/compare_qwen36_tool_http_ab.py" \
    "$RUN_ROOT/baseline/probe.json" \
    "$RUN_ROOT/candidate/probe.json" \
    --candidate-4xx "$RUN_ROOT/candidate/attribution.json" \
