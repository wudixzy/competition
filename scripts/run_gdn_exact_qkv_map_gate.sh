...
if [[ -n "$(git -C "$ROOT" status --porcelain --untracked-files=all -- \
    ...
    echo "exact q/k/v gate refuses a dirty source tree" >&2
    exit 3
fi
...
run_preflight() {
    local name=$1
    timeout --signal=TERM --kill-after=70s 180s \
...
stop_active_group() {
    ...
    if [[ -n "$ACTIVE_PGID" ]]; then
        ...
        echo "tracked PID lacks a verified process group" >&2
        rc=2
    fi
    ACTIVE_PID=""
    ACTIVE_PGID=""
...
run_tracked() {
    ...
    local cleanup_rc=0
    local observed_pgid=""
    setsid timeout --signal=TERM --kill-after=60s "${timeout_s}s" \
    ...
    if [[ -z "$observed_pgid" || "$observed_pgid" != "$ACTIVE_PGID" ]]; then
        echo "$label did not enter an isolated process group" >&2
        return 1
    fi
...
scan_fatal() {
    local pattern
    pattern='CUDA error|SIGSEGV|Fatal Python error|out of memory|device-side assert|Gloo.*(failed|reset|error)|NCCL.*(failed|abort|error)|Connection reset by peer|worker.*(died|lost|exited unexpectedly)|TimeoutError|watchdog.*tim(e|ed) out'
    if grep -Eiq "$pattern" \
...
scan_timeouts() {
    ...
    for file in runtime_identity.rc build.rc benchmark.rc; do
...
report = {
    "schema": "bi100-gdn-exact-qkv-map-runner-v1",
    "version": 1,
    ...
    "artifacts": {
        "extension_sha256": sha256("build/corex_gdn_qkv_map.so"),
...
run_tracked runtime_identity 180 \
...
CURRENT_STAGE=build
run_tracked build 600 \
    bash "$ROOT/tests/build_corex_gdn_qkv_map.sh" "$RUN_ROOT/build"

CURRENT_STAGE=benchmark
run_tracked benchmark 3600 \
    env CUDA_VISIBLE_DEVICES="$GPU_INDEX" \
    python3 "$ROOT/tests/bench_gdn_exact_qkv_map.py" \
    --extension "$RUN_ROOT/build/corex_gdn_qkv_map.so" \
    --device cuda:0 --sequence-steps 1000 \
    --out "$RUN_ROOT/benchmark.json"

CURRENT_STAGE=qualification
if python3 "$ROOT/tests/qualify_gdn_exact_qkv_map.py" \
