...
class FlashInferState(AttentionState):
    ...
    def _get_prefill_wrapper(self):
        if self._prefill_wrapper is None:
            self._prefill_wrapper = None
            # Unsupported BatchPrefillWithPagedKVCacheWrapper Now
...
def unified_flash_infer(
    ...
    if kv_cache.numel() > 0:
        ...
        ops.reshape_and_cache_flashinfer(
            key,
            value,
            kv_cache[:, 0],
            kv_cache[:, 1],
    ...
    if prefill_meta := attn_metadata.prefill_metadata:
        ...
        use_infer_inferface = True
        if use_infer_inferface or kv_cache.numel() == 0:
