    ...
    async def run_engine_loop(engine_ref: ReferenceType):
        ...
        while True:
            if not any(has_requests_in_progress):
                ...
                # may be we do not use this when on server mode(all vllm server in new process, but still without cuda graph)
                # requests_in_progress = [
                #     asyncio.create_task(engine.engine_step(ve))
                #     for ve in range(pipeline_parallel_size)
                # ]
                # has_requests_in_progress = [True] * pipeline_parallel_size
                requests_in_progress = {0: asyncio.create_task(engine.engine_step(0))}
                has_requests_in_progress[0] = True
            ...
            try:
                async with asyncio_timeout(ENGINE_ITERATION_TIMEOUT_S):
                        ...
                        requests_in_progress.values(),
                        return_when=asyncio.FIRST_COMPLETED)
                    for _ in range(len(requests_in_progress)):
                        await asyncio.sleep(0)
                add_virtual_engine = set(list(range(pipeline_parallel_size)))
                for task in done:
                    result = task.result()
                    # virtual_engine = requests_in_progress.index(task)
                    virtual_engine = [k for k,v in requests_in_progress.items() if v == task][0]
                    ...
                    if result or has_unfinished_requests:
                        ...
                        requests_in_progress.pop(virtual_engine)
                        has_requests_in_progress[virtual_engine] = False
                
                # check other engine
                for ve in list(add_virtual_engine - requests_in_progress.keys()):
                    has_unfinished_requests = (
